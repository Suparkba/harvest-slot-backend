...
- `model.joblib` was saved with `scikit-learn 1.8.0`
- `requirements.txt` is aligned to `scikit-learn==1.8.0`
- `model.joblib` and `scikit-learn` version management are backend runtime concerns only. Web/app clients only need to call the API.
